...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!invoices?.length) return jsonResponse({ processed: 0 })

    // Re-run process-invoices
    ...
    const result = res.ok ? await res.json() : { filed: 0, errors: invoices.length }
    return jsonResponse({ processed: result.filed ?? 0 })
  } catch (err) {
